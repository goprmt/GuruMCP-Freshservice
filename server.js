/**
 * Freshservice -> OpenAI Question Builder -> Guru MCP (JSON-RPC over HTTP) -> Freshservice
 *
 * ✅ No MCP SDK dependencies (avoids transport/version churn)
 * ✅ Enforces:
 *    - Company always considered
 *    - Sources only from:
 *        (a) Internal collection (INTERNAL_COLLECTION_ID)
 *        (b) Clients collection (CLIENTS_COLLECTION_ID) AND within matching company board/folder
 *    - Approvals only if: vip=false AND privileged/protected request
 *    - Always check company "Exemptions/Exceptions List" card first
 *
 * Install:
 *   npm i express openai dotenv
 *
 * Env vars (.env recommended):
 *   PORT=8080
 *   FRESHSERVICE_DOMAIN=prometheanit.freshservice.com
 *   FRESHSERVICE_API_KEY=...
 *   GURU_EMAIL=dana@prometheanit.com
 *   GURU_API_TOKEN=...
 *   GURU_AGENT_ID=648a2aa8-2593-4851-b5dd-4f93941b7924
 *   OPENAI_API_KEY=...
 *   OPENAI_MODEL=gpt-5
 */
...
const GURU_AGENT_ID = mustEnv("GURU_AGENT_ID"); // 648a2aa8-2593-4851-b5dd-4f93941b7924
 ...
 * ----------------------------- */
async function freshserviceAddPrivateNote(ticketId, body) {
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      // Freshservice uses Basic Auth; API key as username, X as password
  ...
  if (!res.ok) {
    ...
    throw new Error(`Freshservice note failed ${res.status}: ${text}`);
  }
  return res.json();
}
...
async function guruToolCall({ name, args }) {
  ...
  if (!res.ok) {
    throw new Error(`Guru MCP HTTP ${res.status}: ${text}`);
  }
  ...
  try {
    ...
    throw new Error(`Guru MCP parse error: ${String(e?.message || e)}; body=${text}`);
  }

  if (json.error) {
    throw new Error(`Guru MCP error: ${json.error.message}${json.error.data ? ` — ${json.error.data}` : ""}`);
  }
...
function renderSourcesAsLinks(sources = []) {
  if (!sources.length) return "- _No applicable Guru sources found within allowed scope._";
  ...
  return sources
    .map((s, i) => {
      ...
      const ver = s.verificationState ? ` — ${s.verificationState}` : "";

      // Markdown link; Freshservice renders it nicely
      const linkedTitle = url ? `[${title}](${url})` : title;

      return `${i + 1}. ${linkedTitle}${id}${ver}`;
    })
    .join("\n");
}
  ...
  const resp = await openai.responses.create({
    ...
    text: {
      format: {
        ...
        strict: true,
        schema: schema, // <-- NOTE: schema.schema is the actual JSON Schema object
      },
    },
  });
...
function renderRunbookMarkdown(runbook) {
  ...
  lines.push("");
  lines.push(`**Summary**`);
  lines.push(runbook.summary ? `> ${runbook.summary}` : `> (No summary)`);
  lines.push("");

  if (runbook.prerequisites?.length) {
    lines.push(`**Prerequisites / Info to Confirm**`);
    for (const p of runbook.prerequisites) lines.push(`- ${p}`);
    lines.push("");
  }
  ...
  for (const s of runbook.steps || []) {
    ...
    if (s.verification) lines.push(`   - Verify: ${s.verification}`);
  }
  lines.push("");

  lines.push(`**Approvals**`);
  lines.push(`- Required: ${runbook.approvals?.required ? "Yes" : "No"}`);
  if (runbook.approvals?.rationale) lines.push(`- Rationale: ${runbook.approvals.rationale}`);
  if (runbook.approvals?.who?.length) {
    lines.push(`- Who: ${runbook.approvals.who.join(", ")}`);
  }
  lines.push("");

  if (runbook.escalation?.length) {
    lines.push(`**Escalation**`);
    for (const e of runbook.escalation) lines.push(`- ${e}`);
    lines.push("");
  }

  if (runbook.notes?.length) {
    lines.push(`**Notes**`);
    for (const n of runbook.notes) lines.push(`- ${n}`);
    lines.push("");
  }
 ...
 * Webhook endpoint
 * ----------------------------- */
app.post("/freshservice/webhook", async (req, res) => {
  try {
    ...
    const sourcesBlock = renderSourcesAsLinks(allowedSources);
    const rejectedBlock = rejectedSources.length
      ? rejectedSources
          .map((r) => `- ${r?.source?.title || r?.source?.id || "unknown"}: ${r.reason}`)
          .join("\n")
      : "";
